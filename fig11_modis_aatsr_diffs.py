...
def plot_aatsr_diff(AATSRdata,modeldata,AATSRlon,AATSRlat):
	...
	for i,expi in enumerate(EXPs):
		...
		f.savefig(output_pdf_path+'aatsr/AOD-diff_'+EXP_NAMEs[i]+'-AATSR_2010.pdf')
...
def plot_modis_diff(modisdata,modeldata,modislon,modislat):
	...
	for i,expi in enumerate(EXPs):
		...
		c = plt.colorbar(cs,orientation='horizontal',ticks=bounds,aspect=30,pad=0.05,shrink=0.7)
		c.set_label('AOD bias ['+EXP_NAMEs[i]+'-MODIS]',fontsize=18)
		c.ax.tick_params(labelsize=10)
...
def plot_both(data,bounds=[-0.375,-0.3,-0.225,-0.15,-0.075,-0.025,0.025,0.075,0.15,0.225,0.30,0.375]):
	...
	fcolumn.savefig(output_pdf_path+'bias'+name+'-satellite_2010.pdf')
	fcolumn.savefig(output_png_path+'bias'+name+'-satellite_2010.png',dpi=600)
	return fcolumn,axit
def main(all=False):
	...
	f.savefig(output_png_path+'fig11_bias-NEWSOA-OLDSOA-2satellites_2010.png',dpi=600)
	if all:
