	...
	for i in data.variables:
		if comp in i:
			...
			if initdataset:
				...
				initdataset=False
			#print initdataset,i, np.shape(data.variables[i])
...
def get_gridboxarea(grid, path=''):
	if grid=='TM53x2':
		...
		gridareafile=path+'ec-ei-an0tr6-sfc-glb100x100-0000-oro.nc'
		if os.path.isfile(gridareafile):
			ncgb=nc.Dataset(gridareafile,'r')
			gridarea=ncgb.variables['cell_area']
			return gridarea
		else:
			print 'gridarea file: '+gridareafile+' not found!'
			return False
...
def NME(obs,model):
	...
	NME=nom/denom
	return NME
def read_var(infile,var):
	#data=[]
	attr=[]
	print infile,var
	indataset=nc.Dataset(infile)
	dims=indataset.variables[var].dimensions[:]
	# for dim in dims:
	# 	print len(indataset.dimensions[dim])
	#data=np.zeros(dims)
	#print np.shape(data)
	for i in indataset.variables[var].ncattrs():
		attr.append(indataset.variables[var].getncattr(i))
	#for var in indataset.variables:
	data=indataset.variables[var][:]
	#print np.shape(data),type(data)
	return data,attr

# def monthlengths(year,time):
# 	import calendar
# 	cale=calendar.Calendar()
# 	all=[]
# 	for k in range(12):
# 			f=0
# 			for i in cale.itermonthdays2(2000,k+1):
# 				if i[0]>f:
# 					f=i[0]
# 				#print f
# 			all.append(f)
...
def parse_nas_normalcomments(comments):
	# parse nasa ames comment fields
	# used for reading CN and EMEP OA measurements
